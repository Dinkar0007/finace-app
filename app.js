...
class FinanceStore {
    ...
    withdrawFromSavings(amount) {
        if (isNaN(amount) || amount <= 0) throw new Error('Invalid amount.');
        if (this.state.savings < amount) throw new Error('Insufficient savings vault balance.');
        this.state.savings -= amount;
        this.state.pocket  += amount;
        this.addLog('Withdrawn from Savings Vault', `+₹${amount}`);
        this.saveState();
    }
    ...
    /**
     * BUG FIX: resetData now properly resets both pocket money AND savings vault,
     * along with rent status. This ensures complete data reset when needed.
     */
    resetData() {
        this.state.pocket   = 0;
        this.state.savings  = 0;
        this.state.rentPaid = false;
        this.addLog('App Data Reset', 'All balances cleared');
        this.saveState();
    }
...
class FinanceUI {
    ...
    initEventListeners() {
        ...
        // Withdraw from Vault — also trigger on Enter key
        const withdrawInput = document.getElementById('withdrawAmount');
        document.getElementById('btn-withdraw').addEventListener('click', () => {
            this._handleWithdrawFromVault(withdrawInput);
        });
        withdrawInput.addEventListener('keydown', (e) => {
            if (e.key === 'Enter') this._handleWithdrawFromVault(withdrawInput);
        });
        ...
        document.getElementById('btn-reset').addEventListener('click', () => {
            if (
                confirm(
                    'WARNING: This will reset your Pocket Money and Savings Vault to ₹0, and uncheck Rent. History will remain. Proceed?'
                )
            ) {
    ...
    _handleWithdrawFromVault(input) {
        try {
            this.store.withdrawFromSavings(parseFloat(input.value));
            input.value = '';
            this.render();
            this._flashBalance('pocket');
        } catch (e) {
